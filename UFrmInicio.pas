...
uses
  ...
  Grids, DBGrids, cxDBLabel, MemDS, DBAccess, Uni;
...
type
  ...
  TFrmInicio = class(TdxRibbonForm)
    ...
    dxStatusBarInicioContainer10: TdxStatusBarContainerControl;
    btnLogin: TcxButton;
    zModulos: TUniQuery;
...
begin
  ...
  begin
    ...
    begin
      if zModulos.Locate('NombreForm', acListMenu.Actions[i].Name, [loCaseInsensitive]) or (UpperCase(varGlobal.Elemento('usuario').AsString) = 'ADMIN') then
      begin
  ...
  try
    try
      ...
      begin
        ...
        SetRegistry('\Settings','\' + Mireg + '', 'MysqlProtocol', 'MYSQL');
      ...
      UDMConection.conBD.UserName := VarRegistry('\Settings','\Mi_Equipo', 'MysqlUser');
      ...
      UDMConection.conBD.ProviderName := VarRegistry('\Settings','\Mi_Equipo', 'MysqlProtocol');
...
procedure TFrmInicio.btnLoginClick(Sender: TObject);
begin
  ArrancarSistema;
